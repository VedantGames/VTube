...
const getVideoInPlaylists = asyncHandeller( async (req, res) => {
  ...
  const playlists = user.playlists.filter(playlist => playlist.videos.some(video => video._id == videoId)).map(playlist => playlist._id);
